...
// SUPORTE COMPLETO AO FUNIL DE INSTAGRAM DIRECT + FUNIL WHATSAPP
// ==============================================================================

const SELLER_USER_ID = 'd4444444-4444-4444-4444-444444444444';
const SELLER_NAME = 'EDUARDO';
const SELLER_CREDENTIALS = { email: 'eduardo@piffpaff.com.br', password: 'Eduardo@2026' };
...
// Regra: Cliente novo entra 3 dias seguidos, depois 2x de 7 em 7 dias, depois 1x em 15 dias, depois 1x por mês.
// Exceções (NÃO entra nos 3 dias): Cliente em obra, cliente que deu data, cliente com problema/imprevisto.
// Quando o cliente responde/entra em contato: sai da cobrança e vira atendimento ativo prioritário.
function computeFollowupCadence({ notes, main_objection, commercial_moment, next_best_action, customDate, isNewLead }) {
  ...
  if (customDate && /^\d{4}-\d{2}-\d{2}$/.test(String(customDate).trim())) {
    ...
    // Se o lead for novo e a data fornecida for hoje ou anterior (pois já falou hoje), entra para o dia seguinte (+1 dia)
...
async function getAuthToken(supabaseUrl, publishableKey) {
  ...
  try {
    ...
    console.warn('Erro auth EDUARDO:', e);
  }
...
function isInstagramCard(l) {
  ...
  if (
    ...
    notesStr.includes('migrado com sucesso') ||
    notesStr.includes('migrado para whatsapp') ||
    notesStr.includes('migrado para o whatsapp') ||
    notesStr.includes('conversa ativa pelo whatsapp') ||
    notesStr.includes('negociações whatsapp') ||
    notesStr.includes('negociacoes whatsapp')
  ) {
    return false;
  }
  ...
  const hasPhone = Boolean(cleanPhone(l.phone).length >= 8);
  if (hasPhone && ['investigacao', 'projetos', 'proposta', 'fechamento', 'concluidas'].includes(st)) {
    return false;
  }
  ...
  if (!hasPhone) {
    ...
    if (idLower.startsWith('insta-') || idLower.startsWith('CRM-EDU-') || cInsta.length > 1) return true;
    if (notesStr.includes('direct') || notesStr.includes('anúncio') || notesStr.includes('anuncio')) return true;
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isFindInstagram) {
      ...
      (DEFAULT_INSTAGRAM_LEADS_EDUARDO || []).forEach(instaLead => {
    ...
    if (isListOrCount) {
      ...
      try {
        ...
        console.warn('Erro leitura Supabase EDUARDO:', e);
      }
      ...
      const genericTerms = ['cliente', 'clientes', 'lead', 'leads', 'todos', 'tudo', 'all', 'cadastrado', 'cadastrados', 'geral', 'total', 'EDUARDO'];
      ...
      if (isInstagramQuery) {
        filteredLeads = filteredLeads.filter(isInstagramCard);
      } else if (stageFilter) {
      ...
      if (isInstagramQuery) {
        ...
        message = `EDUARDO possui ${filteredLeads.length} leads no funil de Instagram Direct com pipeline de ${formatBRL(instaTotal)}. ` +
        ...
        message = `${SELLER_NAME} possui atualmente ${mergedLeads.length} clientes cadastrados no CRM (incluindo ${DEFAULT_INSTAGRAM_LEADS_EDUARDO.length} no Instagram Direct) com pipeline total de ${formattedPipeline}. Funil WhatsApp: [${formattedStages}].`;
      }
      ...
      const responseObj = {
        ...
        total_instagram_leads: DEFAULT_INSTAGRAM_LEADS_EDUARDO.length,
    ...
    // 1. Carrega todos os leads do EDUARDO do Supabase para busca profunda e infalível
    let dbLeads = [];
    try {
      ...
      console.warn('Erro ao carregar leads para lookup:', e);
    }
    ...
    // E. Fallback para leads oficiais padrão em memória (se ainda não gravados no Supabase)
    if (!existingLead) {
      const defaultMatch = (DEFAULT_INSTAGRAM_LEADS_EDUARDO || []).find(l => {
    ...
    if (action === 'delete' || action === 'excluir') {
      if (existingLead) {
        try {
          await fetch(`${supabaseUrl}/rest/v1/leads?id=eq.${encodeURIComponent(existingLead.id)}&user_id=eq.${encodeURIComponent(SELLER_USER_ID)}`, {
            method: 'DELETE',
            headers: supabaseHeaders
          });
        } catch (e) {}
      ...
      return res.status(200).json({
        success: true,
        action: 'delete',
        message: `Lead "${name || phone || id || 'selecionado'}" processado no funil de ${SELLER_NAME}!`,
        deleted_lead: existingLead || { id: id || 'temp', name: name || 'Lead' }
      });
    }
    ...
    if (category || commercial_line) {
      payload.commercial_line = category || commercial_line;
    } else if (existingLead && existingLead.commercial_line) {
      payload.commercial_line = existingLead.commercial_line;
    }
    ...
    let resultData = null;
    const existsInDb = existingLead && dbLeads.some(l => l.id === existingLead.id);
    ...
    if (existsInDb) {
      ...
      if (!resultData) resultData = { ...existingLead, ...payload };
    } else {
      payload.id = existingLead?.id || id || (isInstagram ? `insta-${Date.now()}-${Math.random().toString(36).substring(2, 7)}` : `lead-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`);
      payload.name = payload.name || existingLead?.name || (normalizedPhone ? `Cliente (${normalizedPhone})` : (normalizedInsta ? `@${normalizedInsta}` : 'Novo Lead'));
      payload.stage = payload.stage || 'novos';
      payload.temperature = payload.temperature || 'morno';
      payload.commercial_line = payload.commercial_line || 'Linha Premium';
      payload.created_at = existingLead?.created_at || new Date().toISOString();
      ...
      if (!insertRes.ok) {
        ...
        console.error('Erro insert Supabase EDUARDO:', errText);
    ...
    const actionText = existingLead ? (targetStage ? 'reorganizado' : 'atualizado') : 'inserido';
    const followupMsg = `Follow-up: ${payload.followup_date} (${cadence.followup_status}). ${cadence.reason}`;

    return res.status(200).json({
      success: true,
      action: existingLead ? (targetStage ? 'reorganize' : 'update') : 'create',
    ...
    console.error('Erro interno /api/EDUARDO:', err);
