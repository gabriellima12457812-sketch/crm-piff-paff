...
const SELLER_USER_ID = 'c3333333-3333-3333-3333-333333333333';
const SELLER_NAME = 'Felipe';
const SELLER_CREDENTIALS = { email: 'felipe@piffpaff.com.br', password: 'Felipe@2026' };
...
async function getAuthToken(supabaseUrl, publishableKey) {
  ...
  try {
    ...
    console.warn('Erro auth Felipe:', e);
  }
...
function isInstagramCard(l) {
  ...
  if (!hasPhone) {
    ...
    if (idLower.startsWith('insta-') || idLower.startsWith('crm-fel-') || cInsta.length > 1) return true;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isFindInstagram) {
      ...
      (DEFAULT_INSTAGRAM_LEADS_FELIPE || []).forEach(instaLead => {
    ...
    if (isListOrCount) {
      ...
      try {
        ...
        console.warn('Erro leitura Supabase Felipe:', e);
      }
      ...
      const genericTerms = ['cliente', 'clientes', 'lead', 'leads', 'todos', 'tudo', 'all', 'cadastrado', 'cadastrados', 'geral', 'total', 'felipe'];
      ...
      const responseObj = {
        ...
        total_instagram_leads: DEFAULT_INSTAGRAM_LEADS_FELIPE.length,
    ...
    // 1. Carrega todos os leads do Felipe do Supabase para busca profunda e infalível
    let dbLeads = [];
    try {
    ...
    if (!existingLead) {
      const defaultMatch = (DEFAULT_INSTAGRAM_LEADS_FELIPE || []).find(l => {
    ...
    if (existsInDb) {
      ...
      if (!insertRes.ok) {
        ...
        console.error('Erro insert Supabase Felipe:', errText);
    ...
    console.error('Erro interno /api/felipe:', err);
